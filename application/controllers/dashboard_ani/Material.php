...
class Material extends CI_Controller {
	...
	public function __construct()
	{
        ...
        if (!$this->session->has_userdata('active_center_id') ) {
            $this->session->set_flashdata('exception', 'Please select an active center first. Then you can add/ view material to that center. Thank you.');
	...
	public function index()
	{
		...
		#------------------------------------------------#
		$data['materials']  =$this->material_model->read_of_center($this->active_center_id);
		//print_r($data['materials']);
		// print_r($this->organisation);
		// print_r($this->user_id);

		// die();
		$data['content']  = $this->load->view('dashboard_ani/material/list',$data,true);
	...
	public function view($mat_id='')
	{
		...
		$data['content']  = $this->load->view('dashboard_ani/material/view',$data,true);
		$this->load->view('dashboard_ani/layout/main_wrapper_lte',$data);
	}
	...
	public function create(){
		...
		if ($this->form_validation->run() === true) {
			...
			if (empty($id)) {
				...
				redirect('dashboard_ani/material');
			} else { // Update
				...
				redirect('dashboard_ani/material/edit/'.$postData['mat_id']);
			}
		} else {
			$data['content'] = $this->load->view('dashboard_ani/material/upload',$data,true);
			$this->load->view('dashboard_ani/layout/main_wrapper_lte',$data);
		}   
	}
