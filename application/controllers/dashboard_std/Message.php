...
class Message extends CI_Controller {
    ...
    public function index($sender_id =null)
    {
        ...
        $data['content']  = $this->load->view('dashboard_std/messages/inbox' ,$data,true);
    ...
    public function reply($receiver_id = null){
		...
		redirect('dashboard_std//message/new_message');
	}
    ...
    public function sent($receiver_id =null)
	{
		...
		if(!empty($receiver_id)){
			$data['group_by'] = 0;
			$data['messages'] = $this->message_model->sent_of_patient($user_id, $receiver_id);
		}else{
			$data['group_by'] = 1;
			$data['messages'] = $this->message_model->sent_groupby_patient($user_id);
		}
		$data['content'] = $this->load->view('dashboard_std/messages/sent' ,$data,true);
		$this->load->view('dashboard_std/layout/main_wrapper_lte',$data);
	} 
    ...
    {  
        ...
        $data['content'] = $this->load->view('dashboard_std/messages/inbox_information',$data,true);
        $this->load->view('dashboard_std/layout/main_wrapper_lte',$data);
    }
    ...
    {  
        ...
        $data['content'] = $this->load->view('dashboard_std/messages/sent_information',$data,true);
        $this->load->view('dashboard_std/layout/main_wrapper_lte',$data);
    }
    ...
    { 
        ...
        if ($this->form_validation->run() === true) { 
            ...
            // die();
            $data['user_list'] = $this->message_model->user_list($user_id,$users);
            $data['content'] = $this->load->view('dashboard_std/messages/new_message',$data,true);
            $this->load->view('dashboard_std/layout/main_wrapper_lte',$data);
        }  
    }
