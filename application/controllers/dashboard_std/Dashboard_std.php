...
class Dashboard_std extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->model(array(
            'dashboard_model',
            'setting_model',
            'dashboard_std/material_model',
            'dashboard_std/user_model',
            'dashboard_std/message_model',
            'cluster_model',
            'dashboard_std/center_model'
        )); 
        if ($this->session->userdata('isLogIn') == false || $this->session->userdata('user_role') != 5)
            redirect('login');
        $this->user_id = $this->session->userdata('user_id');

        $this->center_id = $this->user_model->read_std_center_id($this->user_id);
        //$this->center_id= null;
        //echo $this->user_id;
        //print_r($this->center_id);die();

        if (count($this->center_id)==0) {
            //top_menu_center_list
            $this->session->set_flashdata('exception', 'No center assigned yet. please contact animator/cluster head/ organisation head/ admin.');
    ...
    public function index()
    {
        ...
        //$center_id = $this->user_model->read_std_center_id($user_id)->center_id;
        $data['new_videos']     =$this->material_model->new_videos($this->user_id,$this->center_id->center_id);
        $data['new_docs']       =$this->material_model->new_docs($this->user_id,$this->center_id->center_id);
        ...
        $data['content']  = $this->load->view('dashboard_std/home',$data,true);
    ...
    {  
        ...
        #------------------------------# 
        $user_id = $this->session->userdata('user_id');
        $data['student']    = $this->user_model->read_by_id($user_id);
		$data['user_role_list']=$this->dashboard_model->get_user_roles();
        $data['content'] = $this->load->view('dashboard_std/profile', $data, true);
        $this->load->view('dashboard_std/layout/main_wrapper_lte',$data);
    }
    ...
    public function form()
    {
        ...
        if ($this->form_validation->run() === true) {
            ...
            redirect('dashboard_std/dashboard_std/profile/');
        } else {
            $user_id = $this->session->userdata('user_id');
