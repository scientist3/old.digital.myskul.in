...
class Material extends CI_Controller {
	...
	public function index()
	{
		...
		#------------------------------------------------#
		$data['materials']  =$this->material_model->read_for_org($this->organisation->org_id);
		//print_r($data['materials']);
		// print_r($this->organisation);
		// print_r($this->user_id);

		// die();
		$data['content']  = $this->load->view('dashboard_org/material/list',$data,true);
	...
	public function view($mat_id='')
	{
		...
		$data['content']  = $this->load->view('dashboard_org/material/view',$data,true);
		$this->load->view('dashboard_org/layout/main_wrapper_lte',$data);
	}  
	public function create(){
		...
		if ($this->form_validation->run() === true) {
			...
			if (empty($id)) {
				...
				redirect('dashboard_org/material');
			} else { // Update
				...
				redirect('dashboard_org/material/edit/'.$postData['mat_id']);
			}
		} else {
			$data['content'] = $this->load->view('dashboard_org/material/upload',$data,true);
			$this->load->view('dashboard_org/layout/main_wrapper_lte',$data);
		}   
	}
